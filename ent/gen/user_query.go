...
type UserQuery struct {
	...
	withPermissions *UserPermissionQuery
	withCarts       *CartQuery
	withOrders      *OrderQuery
	withWishlists   *WishlistQuery
	withReviews     *ReviewQuery
	withVouchers    *UserVoucherQuery
...
func (uq *UserQuery) QueryRoles() *UserRoleQuery {
...
func (uq *UserQuery) QueryPermissions() *UserPermissionQuery {
...
// QueryWishlists chains the current query on the "wishlists" edge.
func (uq *UserQuery) QueryWishlists() *WishlistQuery {
	query := (&WishlistClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryVouchers() *UserVoucherQuery {
	query := (&UserVoucherClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withWishlists:   uq.withWishlists.Clone(),
		withReviews:     uq.withReviews.Clone(),
		withVouchers:    uq.withVouchers.Clone(),
...
func (uq *UserQuery) WithRoles(opts ...func(*UserRoleQuery)) *UserQuery {
	query := (&UserRoleClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithPermissions(opts ...func(*UserPermissionQuery)) *UserQuery {
	query := (&UserPermissionClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithWishlists tells the query-builder to eager-load the nodes that are connected to
// the "wishlists" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithWishlists(opts ...func(*WishlistQuery)) *UserQuery {
	query := (&WishlistClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withWishlists = query
	return uq
}
...
func (uq *UserQuery) WithVouchers(opts ...func(*UserVoucherQuery)) *UserQuery {
	query := (&UserVoucherClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withVouchers = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPermissions; query != nil {
		if err := uq.loadPermissions(ctx, query, nodes,
			func(n *User) { n.Edges.Permissions = []*UserPermission{} },
			func(n *User, e *UserPermission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withWishlists; query != nil {
		if err := uq.loadWishlists(ctx, query, nodes,
			func(n *User) { n.Edges.Wishlists = []*Wishlist{} },
			func(n *User, e *Wishlist) { n.Edges.Wishlists = append(n.Edges.Wishlists, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withVouchers; query != nil {
		if err := uq.loadVouchers(ctx, query, nodes,
			func(n *User) { n.Edges.Vouchers = []*UserVoucher{} },
			func(n *User, e *UserVoucher) { n.Edges.Vouchers = append(n.Edges.Vouchers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadRoles(ctx context.Context, query *UserRoleQuery, nodes []*User, init func(*User), assign func(*User, *UserRole)) error {
...
func (uq *UserQuery) loadPermissions(ctx context.Context, query *UserPermissionQuery, nodes []*User, init func(*User), assign func(*User, *UserPermission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadWishlists(ctx context.Context, query *WishlistQuery, nodes []*User, init func(*User), assign func(*User, *Wishlist)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Wishlist(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.WishlistsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_wishlists
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_wishlists" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_wishlists" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadVouchers(ctx context.Context, query *UserVoucherQuery, nodes []*User, init func(*User), assign func(*User, *UserVoucher)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.UserVoucher(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.VouchersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_vouchers
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_vouchers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_vouchers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
